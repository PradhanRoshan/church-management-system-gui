...
@Component({
  ...
  styleUrls: ['./inactive-members.component.css'],
})
export class InactiveMembersComponent implements OnInit, AfterViewInit {
  ...
  refreshDataTable() {
    this.membersService._allInactiveMembersData$.subscribe({
      next: (data) => {
        console.log(data);
  ...
  openEditDialog(row: any) {
    ...
    setFormControlValue(this.inActiveForm, 'status', 'Active');
  ...
  onDeleteClick() {
    ...
    this.recoverWarning = false;
  }
  onActiveClick() {
    this.onErrorSubmit = false;
    this.showWarning = false;
    this.recoverWarning = true;
  }
  ...
  onUpdateClick() {
    ...
    this.membersService.activeMember(playLoad).subscribe({
      next: (data) => {
        console.log(data);
      },
      error: (err) => {
